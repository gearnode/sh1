...
/*   Updated: 2015/03/04 17:17:22 by bfrimin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static long long	ft_precision_zeros_bin(t_fmt *arg_fmt, long long num_len)
{
	long long nb_zeros;
	...
	if (num_len < arg_fmt->precision)
	{
		...
		ft_print_zeros_bin(nb_zeros, '0');
	}
	return (nb_zeros);
}

static long long	ft_width_spaces_before_bin(t_fmt *arg_fmt,
					long long num_len, unsigned long long arg)
{
		...
		!arg_fmt->flags[1]))
	{
		...
		if (arg_fmt->flags[4] && !arg_fmt->flags[1] &&
			...
			ft_print_zeros_bin(nb_spaces, '0');
		else
			ft_print_zeros_bin(nb_spaces, ' ');
	}
	return (nb_spaces);
}

static long long	ft_width_spaces_after_bin(t_fmt *arg_fmt, long long num_len,
					long long printed)
{
	long long nb_spaces;
					...
					unsigned long long arg)
{
	...
	nb_spaces = 0;
	nb_spaces += ft_width_spaces_before_bin(arg_fmt, num_len, arg);
	nb_spaces += ft_precision_zeros_bin(arg_fmt, num_len);
	return (nb_spaces);
}
					...
					long long printed)
{
	...
	nb_spaces = 0;
	nb_spaces += ft_width_spaces_after_bin(arg_fmt, num_len, printed);
	return (nb_spaces);
}
