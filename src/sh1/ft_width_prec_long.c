...
/*   Updated: 2015/03/04 17:42:35 by bfrimin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
long long			ft_print_zeros_long(long long nb_spaces, char a)
{
	long long i;
...
long long			ft_precision_zeros_long(t_fmt *arg_fmt, long long num_len)
{
	long long nb_zeros;
	...
	if (num_len < arg_fmt->precision)
	{
		...
		ft_print_zeros_long(nb_zeros, '0');
	}
	return (nb_zeros);
}
					...
					long long num_len, long long arg)
{
	...
	if ((sign = ft_needs_sign_before(arg_fmt, arg)))
		ft_print_sign_before_long(arg_fmt, arg, num_len);
	space = ft_print_space_before_long(arg_fmt, arg);
	if (arg_fmt->flags[1])
		nb_spaces = 0;
	else
		...
		!arg_fmt->flags[1])
	{
		if (!arg_fmt->flags[4] ||
			...
			ft_print_zeros_long(nb_spaces, ' ');
		else if (arg_fmt->flags[4] || (arg == 0 && arg_fmt->precision == -1))
			ft_print_zeros_long(nb_spaces, '0');
	}
	sign += ft_print_sign_after_long(arg_fmt, arg, num_len);
